class FixGeoff < ActiveRecord::Migration
  PROJECTS = [
  ...
  def self.up
    PROJECTS.each do |project|
      Project.find_by_name(project.first).update_attributes!(:name => project.first, :url => project[1], :description => project.last)
    end
  end

  def self.down
    PROJECTS.each do |project|
      Project.find_by_name(project.first).update_attributes!(:name => project.first, :url => project.last, :description => project[1])
    end
  end
end
